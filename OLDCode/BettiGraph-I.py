...
path_to_data_folder = "C:\\Users\micha\PycharmProjects\\betti\cloud_data\\"
n = 3
numberOfClouds = 5
numberOfNodes = 10
dimension_of_cloud = 2

# create directed graph with
#G_10_0_1 = create_directed_graph(10)

#create cloud of points
#point_cloud_df = create_cloud_of_points(G_10_0_1,2)

#point_cloud_df.to_csv(path_to_data_folder + "test_cloud" + str(n) + ".csv", index=False)

for cloudNumber in range(numberOfClouds):
    G_temp = create_directed_graph(numberOfNodes)
    temp_cloud = create_cloud_of_points(G_temp,dimension_of_cloud)
    temp_cloud.to_csv(path_to_data_folder + "New_Test_Cloud" + str(cloudNumber) + ".csv", index=False)
